...
module.exports = {
    addReportee: async (req, res) => {
        try {
            ...
            const findDesignation = await models.UserDesignationMapping.findOne({ where: { user_id: id } });
            if (!findDesignation) return res.status(404).json({ response: `Not assign any designation title to user.` });
