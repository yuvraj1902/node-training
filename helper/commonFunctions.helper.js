...
const removeDulicatedFromArrayOfObjects = arrayOfObjects => {
...
const parseLinkedinUrl = data => {
  let linkedinUrls = [];
  const found = data.matchAll(/(http(s)?:\/\/|www+\.|in+\.|)linkedin\.com\/(in\/[A-z0-9_-]+\/?|pub\/[A-z0-9_-]+\/[A-z0-9_-]+\/[A-z0-9_-]+\/[A-z0-9_-]+\/?)/g);
  for (const match of found) {
    try {
      ...
      if (str.includes('linkedin.com/in/')) {
        [, str] = str.split('/in/');
        const index = str.indexOf('/');
        const uniqueIndentifier = str.substring(0, index);
        const url = `https://www.linkedin.com/in/${uniqueIndentifier}/`;
        linkedinUrls = [...new Set([...linkedinUrls, url])];
      } else if (str.includes('linkedin.com/pub/')) {
        [, str] = str.split('/pub/');
        const [uniqueIndentifier, id1, id2, id3] = str.split('/');
        const url = `https://www.linkedin.com/pub/${uniqueIndentifier}/${id1}/${id2}/${id3}/`;
        linkedinUrls = [...new Set([...linkedinUrls, url])];
      }
    } catch (error) {
      console.log(`Error occured while parsing linkedin url\n${error}`);
    }
  }
  return linkedinUrls;
};

const cleanPayload = function (object) {
...
module.exports = {
  ...
  getReasonPhrase,
  generateRandom,
  removeDulicatedFromArrayOfObjects,
  parseLinkedinUrl,
  cleanPayload
};
